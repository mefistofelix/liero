...
export const button=(text:string,action:()=>unknown,cls='')=>{const b=document.createElement('button');b.textContent=text;b.className=cls;b.onclick=()=>Promise.resolve().then(action).catch(report);return b;};
...
export function click(id:string,fn:()=>unknown){el(id).addEventListener('click',()=>Promise.resolve().then(fn).catch(report));}
export function form(id:string,fn:()=>unknown){el<HTMLFormElement>(id).addEventListener('submit',e=>{e.preventDefault();Promise.resolve().then(fn).catch(report);});}
