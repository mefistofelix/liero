import {builtins,catalogLevels,savedLevels,importLevel,levelBytes,thumbnail,type Level} from './maps.ts';
...
export class MapLibrary{
 ...
 setRoom(rotation?:string[]){this.viewingRotation=rotation;input('map-files').disabled=!!rotation;el<HTMLButtonElement>('enable-maps').disabled=!!rotation;this.renderSelection();}
 ...
 constructor(private prefs:Preferences,private save:()=>void,private palette:Uint8Array,private roomActions:{canSwitch:()=>boolean;switchMap:(level:Level)=>Promise<void>}){
  ...
  click('toggle-maps',()=>{this.rotation=this.allSelected()?[]:[...new Set([...this.rotation,...this.levels.map(l=>l.id)])];this.save();this.render();});
 ...
 async init(){this.levels.push(...await catalogLevels(),...await savedLevels());}
 async next(rotation=this.rotation){if(!rotation.length)throw new Error('Enable at least one map in the rotation.');return this.pick(rotation[this.rotationPosition++%rotation.length]);}
 async pick(id:string){const level=this.levels.find(l=>l.id===id);if(!level)throw new Error('A rotation map is missing from this device. Import it again or disable it.');return {level,data:await levelBytes(level)};}
 ...
 render(){
  ...
  el('maps-status').textContent=`${levels.length} maps · ${this.rotation.length} in rotation`;this.renderSelection();el('map-page').textContent=`${this.page+1} / ${pages}`;
  ...
  for(const level of this.visible){const card=document.createElement('article');card.className=`map-card ${this.rotation.includes(level.id)?'selected':''}`;const preview=document.createElement('div');preview.className='preview';preview.textContent=level.id==='random'?'New terrain every round':'Loading preview…';const label=document.createElement('label'),checkbox=document.createElement('input');checkbox.type='checkbox';checkbox.disabled=!!this.viewingRotation;checkbox.checked=this.rotation.includes(level.id);checkbox.onchange=()=>{if(checkbox.checked)this.rotation.push(level.id);else this.rotation=this.rotation.filter(id=>id!==level.id);this.save();this.renderSelection();card.classList.toggle('selected',checkbox.checked);this.renderRotation();el('maps-status').textContent=`${levels.length} maps · ${this.rotation.length} in rotation`;};label.append(checkbox,document.createTextNode(level.name));card.append(preview,label);if(this.roomActions.canSwitch()){const play=button('Play now',()=>this.roomActions.switchMap(level),'play-map');play.setAttribute('aria-label','Play '+level.name+' now');card.append(play);}grid.append(card);
 ...
 private allSelected(){const selected=new Set(this.rotation);return this.levels.every(level=>selected.has(level.id));}
 private renderSelection(){const toggle=el<HTMLButtonElement>('toggle-maps');toggle.disabled=!!this.viewingRotation;toggle.textContent=this.allSelected()?'Deselect all':'Select all';el('rotation-empty').hidden=this.rotation.length>0;}
