...
export function roomWeapons(room:RoomClient,prefs:Preferences,weapons:WeaponLibrary,save:()=>void,update:()=>Promise<void>){
 onMenu('room-weapons-menu',()=>{
  ...
  const list=el('room-weapon-list');list.replaceChildren();el('room-weapon-note').textContent=editable?'Enable the weapons everyone may use. Applies immediately, including weapon bonuses.':'Weapons permitted by this room. Only the host can change them.';
  ...
  const actions=el('room-weapons-actions');actions.replaceChildren();if(editable){actions.append(button('Enable all',()=>{selected=weapons.weapons.map(w=>w.id);list.querySelectorAll<HTMLInputElement>('input').forEach(c=>c.checked=true);}),button('Save weapon availability',async()=>{if(!selected.length){notice('Enable at least one weapon.');return;}prefs.rules.allowedWeapons=[...new Set(selected)];save();if(room.host)await update();notice('Room weapon availability applied.');},'primary'));}
 });
}
