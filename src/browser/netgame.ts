...
export type Frame=[number,number,number,number,number,number,number,Rules?];
 ...
 receive(from:string,packet:Record<string,any>){
  ...
  if((packet.type==='frame'||packet.type==='history')&&!this.room.host&&from===this.room.room?.owner){
   ...
   for(let n=0;n<frames.length;n++){
    const data=frames[n],f=start+n;if(f<this.frame||f>252000||!Array.isArray(data)||![7,8].includes(data.length)||data.length===8&&!validRules(data[7])||!validInput(data.slice(0,3))||!validInput(data.slice(3,6)))continue;
