...
import {validateLevel} from './maps.ts';
...
import {ArenaUI,countryFlag} from './arena-ui.ts';
...
input('player-name').value=prefs.name;input('player-color').value=prefs.color;input('room-name').value=prefs.roomName;input('private-room').checked=prefs.privateRoom;select('region').value=prefs.region;input('keyboard-only').checked=prefs.keyboardOnly;
...
function showGame(){arena.reset();playing=true;el('overlay').hidden=true;el('hud').hidden=localTwo||!!round?.spectator;el('spectator-tools').hidden=!round?.spectator;el<HTMLButtonElement>('record-start').disabled=!recorder.supported();if(!round)closeMenus();}
...
select('region').onchange=()=>{prefs.region=select('region').value;save();refreshRooms();};
...
async function refreshRooms(){
 el('room-browser').hidden=false;el('room-lobby').hidden=true;el('rooms-heading').textContent='Room explorer';const version=++roomListVersion;el('room-list-status').textContent='Refreshing public rooms…';
 const result=await api('/rooms'),region=prefs.region==='auto'?result.region:prefs.region;const rooms:Room[]=result.rooms;
 rooms.sort((a,b)=>Number(b.region===region)-Number(a.region===region)||a.name.localeCompare(b.name));
 if(version!==roomListVersion)return;const list=el('room-list');list.replaceChildren();
 el('room-list-status').textContent=rooms.length?rooms.length+' public rooms · nearest region first · ping to host':'No public rooms yet. Create a room or use Quick match.';
 for(const r of rooms){const tr=document.createElement('tr');const cell=(text:string)=>{const td=document.createElement('td');td.textContent=text;tr.append(td);return td;};cell(r.name);cell(countryFlag(r.country)+' '+r.region);cell(r.count+'/'+r.capacity);const ping=cell('—');ping.title='Direct WebRTC round-trip time to the room host';const rule=cell(modeNames[r.settings.mode]);const subtitle=document.createElement('small');subtitle.textContent=r.settings.rotation.length+' maps · '+r.phase;rule.append(subtitle);cell('').append(button('Watch / join',()=>joinRoom(r.id,'')));list.append(tr);
  const measure=()=>probeRoom(r.id).then(ms=>{if(version===roomListVersion)ping.textContent=ms+' ms';}).catch(()=>{ping.title='Host ping unavailable';ping.replaceChildren(button('Retry ping',measure));});
  if(rooms.indexOf(r)<8)measure();else ping.replaceChildren(button('Measure',measure));
 }
}
...
async function autoConnect(){
 ...
 const result=await api('/rooms'),region=prefs.region==='auto'?result.region:prefs.region;
 const candidates:Room[]=result.rooms.filter((r:Room)=>r.count<r.capacity&&r.id!==room.id);
 for(const group of [candidates.filter(r=>r.region===region),candidates.filter(r=>r.region!==region)]){
  const measured:{room:Room;ping:number}[]=[];let next=0;
  await Promise.all(Array.from({length:Math.min(4,group.length)},async()=>{for(;;){const r=group[next++];if(!r)break;try{measured.push({room:r,ping:await probeRoom(r.id)});}catch{}}}));
...
onMenu('room-settings-menu',()=>{const r=room.room,editable=!r||room.host,rules=r?.settings||prefs.rules;select('game-mode').value=String(rules.mode);input('rule-lives').value=String(rules.lives);input('rule-loading').value=String(rules.loading);input('rule-bonuses').value=String(rules.bonuses);el('settings-form').querySelectorAll<HTMLInputElement|HTMLButtonElement|HTMLSelectElement>('input,button,select').forEach(control=>control.disabled=!editable);el('room-settings-access').textContent=r?countryFlag(r.country)+' '+r.name+' · '+(editable?'You can edit this room.':'View only · only the host can change room rules.'):'Defaults for your next room or local game.';});
async function leaveRoom(){maps?.setRoom(undefined);weapons?.setAvailability(weaponPool(prefs.rules.allowedWeapons));clearTimeout(roundTimeout);roundTimeout=0;recorder.stop();game?.stop();playing=false;round=undefined;setup=undefined;staging=undefined;quickMatching=false;peerLoadouts.clear();peerPings={};await room.leave();history.replaceState(null,'',location.pathname);el('room-browser').hidden=false;el('room-lobby').hidden=true;el('rooms-heading').textContent='Room explorer';el('connection').textContent='Local';el('overlay').hidden=false;el('hud').hidden=true;el('spectator-tools').hidden=true;}
...
click('take-seat',async()=>{await room.call('seat','POST',{play:room.seat<0});notice('Your player slot has been updated for the next round.');});
...
function renderRoom(r:Room){
 if(!r)return;maps?.setRoom(room.host?undefined:r.settings.rotation);weapons?.setAvailability(weaponPool(r.settings.allowedWeapons));arena.room(r,pingFor);el('room-description').textContent=countryFlag(r.country)+' '+r.region+' · '+(r.private?'Private':'Public')+' · '+r.count+'/'+r.capacity;
 const members=el('members');members.replaceChildren();for(const m of r.members){const row=document.createElement('div');row.className='member';const dot=document.createElement('span');dot.className='worm-dot';dot.style.background=m.color;const name=document.createElement('span');name.className='member-name';name.textContent=m.name+(m.id===r.self?' (you)':'');const role=document.createElement('span');role.className='role';role.textContent=m.seat>=0?'Player '+(m.seat+1)+(m.id===r.owner?' / host':''):'Spectator'+(m.id===r.owner?' / host':'');const ping=document.createElement('span');ping.className='ping';const ms=pingFor(m.id);ping.textContent=ms===undefined?'—':ms+' ms';row.append(dot,name,role,ping);members.append(row);}
 ...
 const ping=room.host?0:room.pings.get(r.owner);el('connection').textContent=countryFlag(r.country)+' '+r.name+' · '+r.region+' · '+(ping===undefined?'Connecting…':ping+' ms')+(r.phase==='lobby'?' · Waiting for players':'');
...
async function startOnline(){await guard(async()=>{
 if(!room.host||!room.room)throw new Error('Only the host can start a round.');if(round&&!round.ended)throw new Error('A round is already running.');
 ...
 await saveRoomSettings();const next=await maps.next();const loadouts=players.map(m=>m!.id===room.room!.self?[...prefs.loadouts[0]]:[...peerLoadouts.get(m!.id)!]);
...
room.onPacket=(from,packet)=>{
 try{
  ...
  if(packet.type==='ready'&&room.host){if(packet.version!==1||!validLoadout(packet.loadout))return;peerLoadouts.set(from,packet.loadout);if(round&&!round.ended)sendSetup(from);return;}
...
loadEngine().then(async engine=>{
 module=engine;weapons=new WeaponLibrary(module,prefs,saveWeapons);const ptr=module._liero_palette();roomWeapons(room,prefs,weapons,save,saveRoomSettings);maps=new MapLibrary(prefs,save,module.HEAPU8.slice(ptr,ptr+1024));await maps.init();game=new LocalGame(module,canvas,endGame,renderStats);game.setControls(prefs.controls);game.setSound(prefs.sound);
 ...
 const params=new URLSearchParams(location.hash.slice(1)),id=params.get('room');try{if(id)await joinRoom(id,params.get('invite')||'');else await autoConnect();}catch(error){el('welcome-status').textContent='Could not connect. Browse rooms or play locally.';notice(error instanceof Error?error.message:String(error));}
