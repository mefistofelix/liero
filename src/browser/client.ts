...
function showGame(){profileButtons();arena.reset();playing=true;updateHud();el('spectator-tools').hidden=!round?.spectator;updateRecordingControls();if(!round||el<HTMLDialogElement>('rooms-menu').open)closeMenus();arena.playButton();}
...
function updateRecordingControls(){const control=el<HTMLButtonElement>('record-toggle');control.textContent=recorder.active?'Stop and save':'Record';control.classList.toggle('primary',!recorder.active);control.classList.toggle('danger',recorder.active);setDisabled(control,recorder.saving||(!recorder.active&&(!playing||!recorder.supported())));}
onMenu('recordings-menu',()=>{updateRecordingControls();if(!recorder.supported())el('record-status').textContent='MP4 recording is not supported by this browser. Try a browser with MP4 MediaRecorder support.';return renderArchive();});
click('record-toggle',()=>{if(recorder.active)return recorder.stop();if(!playing)throw new Error('Start or watch a game first.');recorder.start(canvas,module.audioStream,room.room?.name||'Local game');closeMenus();});
recorder.onRecordingChange=recording=>{const icon=el('recordings-toggle');icon.classList.toggle('is-recording',recording);icon.title=recording?'Recording in progress · open recordings':'Recordings';icon.setAttribute('aria-label',icon.title);el('recording-indicator').hidden=!recording;updateRecordingControls();};
recorder.onError=error=>report(error);recorder.onSaved=(recording,persisted)=>{notice(persisted?'Recording saved to the local archive.':'Archive storage is full. Download this recording before closing the page.');renderArchive(recording).catch(report);};
