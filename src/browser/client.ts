...
function showGame(){profileButtons();arena.reset();playing=true;el('hud').hidden=localTwo||!!round?.spectator;el('spectator-tools').hidden=!round?.spectator;setDisabled(el<HTMLButtonElement>('record-start'),!recorder.supported());if(!round||el<HTMLDialogElement>('rooms-menu').open)closeMenus();arena.playButton();}
...
function openProfile(player:number){if(el<HTMLDialogElement>('profile-menu').open){const same=profilePlayer===player;closeMenus();if(same)return;}profilePlayer=player;weapons?.setPlayer(player);select('control-profile').value=player===1?'1':localTwo&&prefs.keyboardOnly?'0':'mouse';const value=player===0?prefs:prefs.player2;input('player-name').value=value.name;input('player-color').value=value.color;el('profile-heading').textContent=localTwo&&!room.id?'Player '+(player+1)+' profile':'Profile';openMenu('profile-menu');}
...
async function startOnline(mapId?:string,restart=false){if(startingRound)return startingRound;const task=guard(async()=>{
 ...
 const chosen=mapId??nextMapId,next=chosen?await maps.pick(chosen):await maps.next(room.room!.settings.rotation);nextMapId=undefined;clearTimeout(roundTimeout);roundTimeout=0;const loadouts=players.map(m=>!m||m.id===room.room!.self?[...prefs.loadouts[0]]:[...peerLoadouts.get(m.id)!]);
