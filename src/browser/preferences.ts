...
export type Preferences={name:string;color:string;roomName:string;region:string;privateRoom:boolean;sound:boolean;keyboardOnly:boolean;loadouts:number[][];rules:Rules;rotation:string[];controls:Controls};
export const defaults:Preferences={name:'Player',color:'#6868fc',roomName:'My room',region:'auto',privateRoom:false,sound:true,keyboardOnly:true,loadouts:[[1,8,15,22,29],[1,8,15,22,29]],rules:{mode:0,lives:15,loading:100,bonuses:4},rotation:['random','temple'],controls:defaultControls};
export function readPreferences():Preferences{
 ...
 return {name:text(value.name,defaults.name,20),color:/^#[a-f0-9]{6}$/i.test(value.color)?value.color:defaults.color,roomName:text(value.roomName,defaults.roomName,48),region:['auto','EU','NA','SA','AS','OC','AF'].includes(value.region)?value.region:'auto',privateRoom:value.privateRoom===true,sound:value.sound!==false,keyboardOnly:value.keyboardOnly!==false,
