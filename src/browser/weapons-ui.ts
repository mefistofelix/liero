...
export class WeaponLibrary{
 weapons:{id:number;name:string;icon:string}[]=[];private slot=0;private dragSlot=-1;private allowed=new Set(Array.from({length:40},(_,i)=>i+1));
 ...
 setPlayer(player:number){select('loadout-player').value=String(player);this.slot=0;}
 ...
 private get player(){return Number(select('loadout-player').value);}
 ...
 render(){
  ...
  const slots=el('loadout-slots');slots.replaceChildren();this.prefs.loadouts[player].forEach((id,slot)=>{
   ...
   b.ondragstart=e=>{this.dragSlot=slot;e.dataTransfer!.effectAllowed='move';e.dataTransfer!.setData('text/plain',String(slot));b.classList.add('dragging');};
   b.ondragover=e=>{if(this.dragSlot>=0){e.preventDefault();e.dataTransfer!.dropEffect='move';b.classList.add('drop-target');}};
   b.ondragleave=()=>b.classList.remove('drop-target');b.ondragend=()=>{this.dragSlot=-1;slots.querySelectorAll('.dragging,.drop-target').forEach(node=>node.classList.remove('dragging','drop-target'));};
   b.ondrop=e=>{e.preventDefault();const from=this.dragSlot;this.dragSlot=-1;this.move(from,slot);};
  ...
  const picker=el('weapon-picker');picker.replaceChildren();for(const w of this.weapons.filter(w=>w.name.toLowerCase().includes(input('weapon-search').value.toLowerCase()))){const b=this.weaponButton(w.id,()=>{this.prefs.loadouts[player][this.slot]=w.id;this.changed();});b.disabled=!this.allowed.has(w.id);if(b.disabled)b.title='Disabled by the room host';picker.append(b);}
 }
}
